...
module.exports = async () => {
    ...
    try {
        ...
        for (let days = 1; days < 2; days++) {
            const time_start = moment().subtract((days + 1), 'days').utc().toISOString();
            const time_end = moment().subtract(days, 'days').utc().toISOString();
            logger.info(`fetching quotes data for dates between ${time_start} and ${time_end}`);
            for (let i = 0; i < coinsIdList.length; i++) {
                coinsIds = coinsIdList[i];
                const quotesHistorical = await coinMarketCapAPI.quotesHistorical(coinsIds.join(','), time_start, time_end);
                var quotes = [];
                coins.forEach(coin => {
                    const id = coin.cmc_id;
                    if (quotesHistorical[id]) {
                        var coinQuote = quotesHistorical[id];
    ...
    logger.info('loadQuotesHistory process stop');
};